...
static QIcon makeIcon(const QString& type) {
    ...
    if (type == "立方体") {
        ...
        // ============================================================
        // 【占位图标】"全部构建": 立方体线框 + 左下对勾
        // TODO: 替换为正式图标 — 只需改写本分支内的绘制代码
        // ============================================================
        drawBoxWire(p, {11, 2.5}, {17.5, 6}, {11, 9.5}, {4.5, 6}, 3.0, 5.8);   // 立方体线框
        p.setPen(QPen(QColor(46, 160, 67), 2.2));                              // 绿色对勾
        p.drawPolyline(QPolygonF({QPointF(2.5, 12.5), QPointF(5.5, 15.5), QPointF(10, 10)}));
        p.setPen(solid);
    } else if (type == "导入几何") {
        // ============================================================
        // 【占位图标】"导入几何": 容器框 + 向下进入的箭头
        // TODO: 替换为正式图标 — 只需改写本分支内的绘制代码
        // ============================================================
        p.drawRect(QRectF(4, 8, 12, 9));            // 容器框 (下部)
        p.drawLine(QPointF(10, 2.5), QPointF(10, 7));   // 竖线进入容器
        drawArrowHead(p, {10, 7.5}, {0, 1});            // 向下箭头
    } else if (type == "导出几何") {
        // ============================================================
        // 【占位图标】"导出几何": 容器框 + 向上离开的箭头
        // TODO: 替换为正式图标 — 只需改写本分支内的绘制代码
        // ============================================================
        p.drawRect(QRectF(4, 8, 12, 9));            // 容器框 (下部)
        p.drawLine(QPointF(10, 4.2), QPointF(10, 8));   // 竖线离开容器
        drawArrowHead(p, {10, 3.0}, {0, -1});           // 向上箭头
    } else {
        // 未知类型: 问号占位
