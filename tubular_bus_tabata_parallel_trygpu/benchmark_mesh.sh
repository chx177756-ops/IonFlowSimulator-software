...
printf "%-3s %-10s %-4s %-10s %-10s\n" "NP" "MeshSize" "Run" "ConvTime_s" "LoadTime_s" | tee "$RESULT_FILE"
...
for np_dir in "$MESH_DIR"/*/; do
    ...
    for mesh_dir in "$np_dir"*/; do
        ...
        if [ "$np" -eq 1 ]; then
            # np=1: 单文件 mesh.msh
            files=$(ls "$mesh_dir"*.msh 2>/dev/null | grep -v "^\._")
            nfiles=$(echo "$files" | wc -w)
        else
            # np>1: 分片文件 mesh_1.msh mesh_2.msh ...
            files=$(ls "$mesh_dir"mesh${mesh}_*.msh 2>/dev/null | grep -v "^\._" | sort -V)
            nfiles=$(echo "$files" | wc -w)
        fi

        if [ "$nfiles" -ne "$np" ]; then
            echo "  [跳过] NP=$np Mesh=$mesh: 文件数=$nfiles, 期望=$np"
            continue
        fi

        prefix="$mesh_dir/mesh${mesh}.msh.part"

        echo ""
        echo "=== NP=$np, Mesh=$mesh ==="
...
with open('$CONFIG_TMP') as f:
    c = json.load(f)
c['mesh_file'] = '${mesh_dir}/mesh${mesh}.msh'
...
" 2>/dev/null
            ...
            printf "%-3d %-10d %-4d %-10s %-10s\n" $np $mesh $run "$t_conv" "$t_load" | tee -a "$RESULT_FILE"
        done
    done
done
...
print("\n========== 平均值汇总 ==========")
print(f"{'NP':<4} {'Mesh':<10} {'Conv_avg(s)':<12} {'Load_avg(s)':<12}")
for (np, mesh), data in sorted(results.items()):
    ...
    print(f"{np:<4} {mesh:<10} {conv_str} {la:<12.4f}")
PYEOF "$RESULT_FILE"
