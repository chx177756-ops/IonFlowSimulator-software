    ...
    t_output.Start();
    {
        ...
        if (export_txt) {
            ...
            if (my_rank == 0)
            {
                ...
                struct PointData { double rho, E, E0; };
                std::vector<PointData> serial_data(NV_serial, {0.0, 0.0, 0.0});

                struct IntCoord { int64_t x, y, z; bool operator==(const IntCoord& o) const { return x==o.x && y==o.y && z==o.z; } };
                struct IntCoordHash { std::size_t operator()(const IntCoord& k) const { return std::hash<int64_t>()(k.x) ^ (std::hash<int64_t>()(k.y) << 1) ^ (std::hash<int64_t>()(k.z) << 2); } };

                std::unordered_map<IntCoord, size_t, IntCoordHash> coord_map;
                coord_map.reserve(total_entries);
                for(size_t k = 0; k < total_entries; ++k) {
                    IntCoord key{ (int64_t)std::round(recv_coords[3*k+0] * 1e5), (int64_t)std::round(recv_coords[3*k+1] * 1e5), (int64_t)std::round(recv_coords[3*k+2] * 1e5) };
                    coord_map[key] = k;
                }

                int matched_count = 0;
                for (int s_idx = 0; s_idx < NV_serial; ++s_idx) {
                    const double* v = serial_mesh.GetVertex(s_idx);
                    IntCoord key{ (int64_t)std::round(v[0] * 1e5), (int64_t)std::round(v[1] * 1e5), (int64_t)std::round(v[2] * 1e5) };
                    auto it = coord_map.find(key);
                    if (it != coord_map.end()) {
                        size_t k = it->second;
                        serial_data[s_idx].rho = recv_rho[k]; serial_data[s_idx].E = recv_E[k]; serial_data[s_idx].E0 = recv_E0[k];
                        matched_count++;
                    }
                }
                std::cout << "[Output] Matched " << matched_count << " / " << NV_serial << " vertices for TXT export." << std::endl;

                std::vector<double> final_rho(NV_serial, 0.0), final_E(NV_serial, 0.0), final_E0(NV_serial, 0.0);
                for(int i=0; i<NV_serial; ++i) {
                    final_rho[i] = serial_data[i].rho; final_E[i] = serial_data[i].E; final_E0[i] = serial_data[i].E0;
                }
